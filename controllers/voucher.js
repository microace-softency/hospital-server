...
router.delete("/removevoucher/:id", (req, res) => {
  const { id } = req.params;
  const sqlRemove = "DELETE FROM vouchers WHERE id = ?";
...
router.post("/createvoucher", async (req, res) => {
  try {
    ...
    const sqlInsert =
      "INSERT INTO vouchers (TYPE, TRNNO, TRN_DATE, GLCODE,DESCRIPT, CBNAME,NARRATION, PAYEE_R,CHEQUE_ON,CHEQUE_NO, CHEQUE_DT, AMOUNT) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
router.get("/:id", async (req, res) => {
  const { id } = req.params;
  const sqlGet = "SELECT * FROM vouchers WHERE id = ?";
...
router.put("/updatevoucher/:id", async (req, res) => {
  ...
  const sqlUpdate =
    "UPDATE vouchers SET TYPE = ?, TRNNO = ?, TRN_DATE = ?, GLCODE = ?, DESCRIPT = ?, CBNAME = ?, NARRATION = ?, PAYEE_R = ?, CHEQUE_ON = ?, CHEQUE_NO = ?, CHEQUE_DT = ?, AMOUNT = ?  WHERE id = ?";
  await db.query(
    sqlUpdate,
    [
      TYPE,
